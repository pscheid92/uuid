...
func FuzzDecodersAgree(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, s string) {
		...
		// Whatever ParseLenient accepts, in any form, must round-trip
		// through the canonical string.
		if lenientErr == nil {
			if again, err := Parse(lenient.String()); err != nil || again != lenient {
				t.Fatalf("round-trip Parse(%q) = %s, %v; want %s", lenient.String(), again, err, lenient)
			}
		}
