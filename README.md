...
Every error for malformed input, whether from parsing, JSON or binary decoding, or `Scan`, matches `uuid.ErrInvalid`. Use `errors.AsType[*uuid.ParseError]` when you need the offending input:
